...
<div class="container">
    ...
    <div class="row center">
        <div class="col s12">
            <ul class="tabs">
                <li class="tab col s6"><a class="active" href="#atendimentos">Atendimentos Sem 'Status'</a></li>
                <li class="tab col s6"><a href="#configs">Configurações</a></li>
            </ul>
        </div>
        <div id="atendimentos" class="col s12">
            @if(count($datas) == 0)
            <br>
            ...
            <form method="POST" action="{{ route('updateCompany') }}">
                <div class="input-field col s12">
                    ...
                    <div class="switch">
                        <label>
                            Off
                            <input type="checkbox" name="view_calendar" id="viewCalendar">
                ...
                <div class="input-field col s12">
                    <span>Bloquear (Próximas) Vendas com determinado Status</span>
                    <div class="switch">
                        <label>
                            Off
                            <input type="checkbox" name="control_sale_status" id="controlSaleStatus">
                            <span class="lever"></span>
                            On
                        </label>
                    </div>
                ...
                <div class="input-field col s12">
                    <select class="select2 browser-default" name="status_id" id="status">
                        <option value="" selected>Sem status</option>
                        @foreach ($statuses as $status)
                        <option value="{{$status->id}}">
                            {{$status->name}}
                ...
                <div class="row">
                    <div class="s12 center">
                        <button class="btn-small waves-effect" type="submit">Salvar todos</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        let company = JSON.parse('<?= $company; ?>');
        
        let viewCalendar = company.view_calendar;
        if (viewCalendar) {
            $("#viewCalendar").prop('checked', true);
        } else {
            $("#viewCalendar").prop('checked', false);
        }

        let controlSaleStatus = company.control_sale_status;
        if (controlSaleStatus) {
            $("#controlSaleStatus").prop('checked', true);
        } else {
            $("#controlSaleStatus").prop('checked', false);
        }
       
        let $statusId = company.status_id;
        
        $('#status option[value="' + $statusId + '"]').attr('selected', true);
        $('#status').change();
        $('#status').formSelect();
    });
